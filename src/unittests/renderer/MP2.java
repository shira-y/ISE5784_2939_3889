...
public class MP2 {
	...
	/**
	 * Test method for rendering a scene with over 100 objects and more than 5 light
	 * sources.
	 */
	@Test
	public void MultipleObjectsTest() {
		...
		Camera.Builder camera = Camera.getBuilder().setLocation(new Point(-55, 8, -55))
				.setDirection(towardsScene, trueUpVector).setVpDistance(150).setVpSize(80, 80);
		...
		for (int i = 0; i < 10; i++) {
			for (int j = 0; j < 10; j++) {
				// Create a triangle
				scene.geometries.add(new Triangle(new Point(-40 + i * 8, 0, -40 + j * 8),
						new Point(-36 + i * 8, 2, -40 + j * 8), new Point(-32 + i * 8, 0, -40 + j * 8))
						.setEmission(new Color(i * 25, j * 25, (i + j) * 12)).setMaterial(shinyMaterial));

				// Create a sphere
				scene.geometries.add(new Sphere(new Point(-36 + i * 8, 3, -36 + j * 8), 1)
						.setEmission(new Color((i + j) * 12, i * 25, j * 25)).setMaterial(shinyMaterial));
			}
		}
		...
		scene.lights.add(new SpotLight(new Color(0, 255, 0), new Point(-30, 30, -35), new Vector(1, -1, 0))
				.setKl(0.00001).setKq(0.000005));
		scene.lights.add(new PointLight(new Color(0, 0, 255), new Point(-50, 20, -50)).setKl(0.00001).setKq(0.000005));
		scene.lights.add(new SpotLight(new Color(255, 255, 0), new Point(-10, 40, -10), new Vector(0, -1, 0))
				.setKl(0.00001).setKq(0.000005));
